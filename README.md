...
**架构升级**(2026-07-13):视觉塔 CLIP-B/32→**CLIP-ViT-L/14@336px** (768→1024 维, 49→576 visual tokens),
LLM SmolLM2-360M→**Qwen2.5-0.5B-Instruct** (LLaVA 官方选型)。projector 随之重训。

| 组件 | 参数量 | 状态 |
|-----|--------|------|
| CLIP-ViT-L/14@336 (视觉塔) | ~304M | 冻结 |
| Qwen2.5-0.5B-Instruct (LLM) | ~494M | 冻结 |
| MLP Projector (1024→2048→896) | **3.94M** | 唯一可训 |
| 总 / 可训比 | ~801M / 3.94M | **0.49%** |
...
| 指标 | 值 | 说明 |
|------|----|------|
| **CIDEr** | **0.927**(×100 惯例 92.7) | 🎯 现代 captioning 主指标 |
| **BLEU-4**(官方) | **31.73%** | 与本项目手写 corpus BLEU-4 **31.76% 几乎一致**(差 0.03,验证自写口径准确) |
| BLEU-1/2/3 | 76.2 / 58.9 / 43.8 | |
| METEOR / ROUGE-L | 0.274 / 0.564 | |

对比:Show-Attend-Tell Flickr8k BLEU-4 ~19.5(soft)/21.3(hard) → **我们 31.7 明显超出约 10 点**(同数据集/同指标/同 5 参考)。

> 这是本项目**方法学最干净、最可对标**的数字:proper Karpathy split、从零训练、5 参考、无 train-on-test,且用官方工具链复核。
> **诚实标注**:CIDEr **跨数据集不可比**(COCO SOTA CIDEr 130-155 是另一语料的 TF-IDF,不能与 Flickr8k 的 92.7 相减);
> 我们超越的是**同数据集的 2015 经典模型**,靠 CLIP-L+Qwen 预训练杠杆。全指标见 [`docs/benchmark_landscape.md`](docs/benchmark_landscape.md)。
> **为何超越 2015 经典模型**:站在 CLIP-L/14 + Qwen2.5 预训练主干肩上(冻结)、只训 3.94M projector —— 这正是项目论点(现代预训练 + 微型 adapter),不是刷 trick。
...
Image → CLIP-ViT-L/14@336 (冻结) → [B, 576, 1024] visual features
                                ↓
                         2 层 MLP Projector (唯一可训层, 3.94M 参数)
                                ↓
                         [B, 576, 896] projected tokens
                                ↓
Text prompt (含 <image> 占位符) ─┬──→ 拼装为 [text_pre] [visual] [text_post]
                                ↓
                    Qwen2.5-0.5B-Instruct (冻结) → text output
```
...
# 训练 (当前 SOTA 配方: CLIP-L/14@336 + Qwen2.5-0.5B-Instruct, 从 base ckpt 微调)
python train.py --data data/flickr_1k/flickr_1k_train.jsonl \
                ...
                --out checkpoints/projector_L14_qwenInstruct_ft.pt

# 评测 (corpus + sentence BLEU-4) / 推理
python evaluate.py --ckpt checkpoints/projector_L14_qwenInstruct_ft_best.pt --max-samples 100
python inference.py --ckpt checkpoints/projector_L14_qwenInstruct_ft_best.pt \
                    --image data/flickr_1k/images/<name>.jpg --dtype bf16 --temperature 0.0
```

**模型首次需从 HuggingFace 下载**(之后 `~/.cache/huggingface/` 复用):
CLIP-ViT-L/14@336 (~1.7 GB) · Qwen2.5-0.5B-Instruct (~1 GB) · CLIP-B/32 & SmolLM2-360M (前期基线用)。
